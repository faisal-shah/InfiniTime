...
namespace {
  ...
  constexpr ble_uuid128_t stepCountYesterdayCharUuid {CharUuid(0x03, 0x00)};
...
MotionService::MotionService(NimbleController& nimble, Controllers::MotionController& motionController)
  : nimble {nimble},
    ...
    characteristicDefinition {{.uuid = &stepCountCharUuid.u,
                              ...
                              {.uuid = &stepCountYesterdayCharUuid.u,
                               .access_cb = MotionServiceCallback,
                               .arg = this,
                               .flags = BLE_GATT_CHR_F_READ,
                               .val_handle = &stepCountYesterdayHandle},
                              {0}},
    serviceDefinition {
...
int MotionService::OnStepCountRequested(uint16_t attributeHandle, ble_gatt_access_ctxt* context) {
  ...
  if (attributeHandle == stepCountYesterdayHandle) {
    uint32_t buffer = motionController.NbSteps(MotionController::Days::Yesterday);
